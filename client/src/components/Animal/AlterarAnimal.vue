...
export default {
  ...
  methods: {
    onSubmit (event) {
      ...
      console.log(animal)
      axios.put('http://localhost:3000/animais/' + this.$route.query.id, animal)
        .then(animal => {
          alert('Animal Atualizado na Base de Dados!')
  ...
  async created () {
    ...
    try {
      this.animal = await axios.get('http://localhost:3000/animais/' + this.$route.query.id)
      this.animal = this.animal.data

      this.form.fk_id_pessoa = this.animal.fk_id_pessoa
      this.form.id_fazenda = this.animal.id_fazenda
      this.form.no_animal = this.animal.no_animal
      this.form.no_raca = this.animal.no_raca
      this.form.vr_peso = this.animal.vr_peso
      this.form.sexo = this.animal.sexo
      this.form.dt_nascimento = this.animal.dt_nascimento
    } catch (err) {
      console.log(err)
    }
  }
